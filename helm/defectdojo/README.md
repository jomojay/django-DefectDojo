...
|------------|------|---------|
| oci://registry-1.docker.io/cloudpirates | valkey | 0.22.1 |
...
| featureFlags | object | `{"rbac":"shadow"}` | Feature rollout flags. Like the `sso` block above, these are injected into the uwsgi, celery worker, celery beat and initializer containers alike and must match across all of them: celery tasks resolve permissions too, and a flag that only reached the web tier would make the workers disagree with the web pods about who can see what. |
| featureFlags.rbac | string | `"shadow"` | Role-based access control enforcement state. One of: `off` - legacy Authorized Users / staff authorization only, exactly the pre-RBAC engine. `shadow` (default) - both the legacy and the role-aware resolver run on every object-level check, the legacy answer is still the one returned, and any disagreement is logged at WARNING with the user, the object type and pk, the permission, and both answers. Nobody gains or loses access in this state; it is a dark launch that surfaces every access change under real traffic before anyone is affected by it. Follow it in `kubectl logs -l defectdojo.org/component=django -f`, where each divergence is prefixed `RBAC shadow divergence`. `on` - the role-aware resolver becomes authoritative for both object-level checks and queryset filtering. Flip to this only after the shadow log has been reviewed. Any other value fails the render rather than silently falling back, because DefectDojo itself fails closed to `off` and a typo would quietly disable enforcement. Quote it when you set it to on in a values file — `rbac: on` is a YAML boolean and is rejected as the wrong type, `rbac: "on"` is what you want. |
...
| sso | object | `{"azureAd":{"clientId":"","clientSecret":{"secretKey":"DD_SOCIAL_AUTH_AZUREAD_TENANT_OAUTH2_SECRET","secretName":"defectdojo-sso-azuread"},"enabled":false,"redirectIsHttps":true,"tenantId":"","whitelistedDomains":""},"classicAuthEnabled":true}` | Single sign-on configuration. Every value here is per-deployment: point the chart at whichever identity provider tenant this instance should trust. Nothing is enabled by default, so leaving this block untouched keeps the instance on local username/password login only. The settings below are injected into the uwsgi, celery worker, celery beat and initializer containers alike. They must match across all of them: the initializer runs `manage.py migrate`, and the `social_django` tables are only created when the flag is on there too. |
| sso.azureAd.clientId | string | `""` | Application (client) ID of the app registration. |
| sso.azureAd.clientSecret | object | `{"secretKey":"DD_SOCIAL_AUTH_AZUREAD_TENANT_OAUTH2_SECRET","secretName":"defectdojo-sso-azuread"}` | The client secret is never a Helm value. Create the Secret out of band (or through your secret manager / External Secrets / Sealed Secrets) and reference it here; the chart injects it with `valueFrom.secretKeyRef`. Pods will not start until the Secret exists, which is the intended fail-closed behaviour. |
| sso.azureAd.enabled | bool | `false` | Enable Microsoft Entra ID (Azure AD) OIDC login. |
| sso.azureAd.redirectIsHttps | bool | `true` | Force the OAuth2 redirect_uri to be built with https. Needed when DefectDojo sits behind a TLS-terminating proxy that does not forward X-Forwarded-Proto, because the identity provider compares the redirect_uri against the registered reply URL exactly. |
| sso.azureAd.tenantId | string | `""` | Directory (tenant) ID of the app registration. |
| sso.azureAd.whitelistedDomains | string | `""` | Comma separated list of email domains permitted to sign in, e.g. `one.example,two.example`. Leave empty to rely solely on the single-tenant app registration. An empty list also disables linking an SSO identity to a pre-existing local DefectDojo account. |
| sso.classicAuthEnabled | bool | `true` | Local username/password login. Keep this enabled: it is the break-glass path when the identity provider is unreachable, and turning it off can lock every user out of the instance. |
